...
if Proses:
    ...
    avg_color_per_row_RGB = np.average(img_RGB, axis=0)
    ...
    x_train_YCBCR, x_test_YCBCR, y_train_YCBCR, y_test_YCBCR = train_test_split(x_YCBCR, y_YCBCR, test_size=0.2)
    scaler = StandardScaler()  
    scaler.fit(x_train_YCBCR)
    x_train_YCBCR = scaler.transform(x_train_YCBCR)  
    x_test_YCBCR = scaler.transform(x_test_YCBCR)
    knn_YCBCR = KNeighborsClassifier(n_neighbors=5)
    ...
    x_train_HSV, x_test_HSV, y_train_HSV, y_test_HSV = train_test_split(x_HSV, y_HSV, test_size=0.2)
    scaler = StandardScaler()  
    scaler.fit(x_train_HSV)
    x_train_HSV = scaler.transform(x_train_HSV)  
    x_test_HSV = scaler.transform(x_test_HSV)
    knn_HSV = KNeighborsClassifier(n_neighbors=5)
   ...
   # knn using color model YUV
    ...
    x_train_YUV, x_test_YUV, y_train_YUV, y_test_YUV = train_test_split(x_YUV, y_YUV, test_size=0.2)
    scaler = StandardScaler()  
    scaler.fit(x_train_YUV)
    x_train_YUV = scaler.transform(x_train_YUV)  
    x_test_YUV = scaler.transform(x_test_YUV)
    knn_YUV = KNeighborsClassifier(n_neighbors=5)
    ...
    x_train_LAB, x_test_LAB, y_train_LAB, y_test_LAB = train_test_split(x_LAB, y_LAB, test_size=0.2)
    scaler = StandardScaler()  
    ...
    x_train_XYZ, x_test_XYZ, y_train_XYZ, y_test_XYZ = train_test_split(x_XYZ, y_XYZ, test_size=0.2)
    scaler = StandardScaler()  
